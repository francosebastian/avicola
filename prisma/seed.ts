...
async function main() {
  ...
  for (let sem = 18; sem <= 90; sem++) {
    ...
    await prisma.curvaEstandar.create({ data: { lineaGenetica: "Lohmann LSL", semanaVida: sem, posturaEsperada: Math.round(pico * 10) / 10, mortalidadEsperada: Math.max(0.05, 0.12 + (sem - 18) * 0.007), consumoEsperadoGramos: Math.round(95 + sem * 0.5), pesoHuevoEsperado: Math.round((57 + sem * 0.12) * 10) / 10, pesoCorporalEsperado: Math.round((1450 + sem * 11) * 10) / 10 } })
  }
  ...
  const l3 = await prisma.lote.create({ data: { codigoLote: "H-034", seccionId: g2n.id, lineaGenetica: "Lohmann LSL", proveedorPollita: "AgroAvícola del Sur", cantidadInicial: 5000, fechaRecepcion: new Date("2026-04-01"), fechaNacimiento: new Date("2026-03-29"), pesoInicialPromedio: 34.5, costoPollitaUnitario: 3700, estado: "recria" } })
  const l4 = await prisma.lote.create({ data: { codigoLote: "H-035", seccionId: g2s.id, lineaGenetica: "Hy-Line Brown", proveedorPollita: "Avícola El Carmen", cantidadInicial: 5000, fechaRecepcion: new Date("2025-07-28"), fechaNacimiento: new Date("2025-07-25"), pesoInicialPromedio: 35.5, costoPollitaUnitario: 3850, estado: "postura" } })
  ...
  await prisma.formulaAlimento.create({ data: { nombre: "Postura Fase 1 — Hy-Line Brown", tipoAlimento: "postura_1", ingredientes: [{ ingrediente: "Maíz", porcentaje: 62.5 }, { ingrediente: "Soya", porcentaje: 22.0 }, { ingrediente: "Conchuela", porcentaje: 8.5 }, { ingrediente: "Vitaminas", porcentaje: 0.5 }, { ingrediente: "Minerales", porcentaje: 1.5 }, { ingrediente: "Calcio", porcentaje: 5.0 }], costoKgEstimado: 0.385, proteinaBruta: 17.5, energiaMetabolizable: 2850 } })
